...
# lakes to explore
lake_ids <- c("baldegg", "biel", "brienz", "greifen", "hallwil", 
              "murten", "neuenburg", "sempach", "thun", "zurich")
# compute trend in phosphate and plot each time series
data_list <- list()
...
# for each data set perform leave-one-out predictions for each species ------------------------------
# data frame to store R2 results
results_df <- data.frame()
# lakes to use
lakes_to_use <- 10
lakes_names <- lake_ids[lakes_to_use]
...
# loop over lakes
for (i in 1:length(lakes_to_use)) {
  # current data set
  training_df <- data_list[[lakes_to_use[i]]]
  ...
  if (save_plot) {
    if (filter_phosphate) {
      ggsave(paste("figs/fig5/fig_lake_plankton_", lakes_names[i], "_ts_", temporal_resolution, "_filter_phosphate.pdf", sep = ""), 
             fig, width = 16, height = 10, units = "in")
    } else {
      ggsave(paste("figs/fig5/fig_lake_plankton_", lakes_names[i], "_ts_", temporal_resolution, ".pdf", sep = ""), 
             fig, width = 16, height = 10, units = "in")
    }
  }
  ...
  for (j in 1:length(sp_input)) {
    ...
    curr_df <- data.frame(lake = lakes_names[i],
                          species = sp_input[j],
                          E_sp = E_sp,
                          phi_sp = phi_sp,
    ...
    fig <- ggplot(data = plot_df) +
      ...
      ggtitle(label = paste("Lake ", lakes_names[i], ", ", sp_input[j], ", R2 = ", round(max(R2), 2), sep = ""),
    ...
    # save plot
    if (save_plot) {
      ggsave(paste("figs/fig5/fig_lake_plankton_", lakes_names, "_", sp_input[j], "_loo_predictions_", temporal_resolution, 
